pipeline {
    ...
    environment {
        ...
        PROGRAM_ID = "${env.GITHUB_ENV.PROGRAM_ID}"
    }
    ...
    stages {
        stage('Checkout') {
            steps {
                script {
                    checkout scmGit(branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[credentialsId: 'jenkins-demo', url: 'https://github.com/sloweyyy/Trustify-Backend.git']])
                }
            }
        }
        ...
        stage('Build Docker Image') {
            steps {
                script {
                    if (isUnix()) {
                        sh 'docker build -t trustify-backend .'
                    } else {
                        bat 'docker build -t trustify-backend .'
                    }
                }
            }
        }
        ...
        stage('Push Docker Image') {
            steps {
                script {
                    if (isUnix()) {
                        sh 'docker tag trustify-backend ${env.GITHUB_ENV.DOCKER_USERNAME}/trustify-backend'
                        sh 'docker push ${env.GITHUB_ENV.DOCKER_USERNAME}/trustify-backend'
                    } else {
                        bat 'docker tag trustify-backend %DOCKER_USERNAME%/trustify-backend'
                        bat 'docker push %DOCKER_USERNAME%/trustify-backend'
                    }
                }
            }
        }
