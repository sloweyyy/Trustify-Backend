...
const getHistoryWithStatus = async (userId, role = 'user') => {
  ...
  // Add sorting by createdAt for user role
  if (role === 'user') {
    aggregationPipeline.push({ $sort: { createdAt: -1 } });
  }

  const history = await Document.aggregate(aggregationPipeline);
...
const autoVerifyDocument = async () => {
  try {
    ...
    const updatePromises = pendingDocuments.map(async (tracking) => {
      ...
      const hasIPFSFile = document.files.some(
        (file) => file.firebaseUrl && file.firebaseUrl.includes('https://gateway.pinata.cloud/ipfs/')
      );
      ...
      if (!document || !document.notarizationService || !document.notarizationService.required_documents) {
