...
const joinSession = async (sessionId, action, userId) => {
  ...
  console.log(
    'Session users:',
    session.users.map((u) => ({ email: u.email, status: u.status }))
  );
  console.log('Current user email:', user.email);

  const userIndex = session.users.findIndex((userItem) => userItem.email.toLowerCase() === user.email.toLowerCase());
  ...
  try {
    ...
    if (session.output && Array.isArray(session.output) && session.output.length > 0) {
      for (const outputFile of session.output) {
        ...
        // For backward compatibility, use either transactionHash or transactionSignature
        const txId = nftData.transactionHash || nftData.transactionSignature;
        ...
        // Update output file with transaction details - support both Ethereum and Solana fields
        outputFile.transactionHash = transactionData.transactionHash;

        // Add Solana specific fields
        if (nftData.mintAddress) {
          outputFile.mintAddress = nftData.mintAddress;
        }

        if (nftData.transactionSignature) {
          outputFile.transactionSignature = nftData.transactionSignature;
        }
...
const autoVerifySession = async () => {
  try {
    ...
    const updatePromises = pendingDocuments.map(async (tracking) => {
      ...
      if (!session || !session.notaryService || !session.notaryService.required_documents) {
